...
for i in range(1, total_raids):
    img = process_img.read_image(i)

    processed_img = process_img.remove_bottom_bar(img)
    processed_img = process_img.crop_resize_img(img)
    ...
    pokemon_name_img = process_img.crop_pokemon_name(processed_img)

    thresh = process_img.get_threshold(pokemon_name_img, 240, True)
